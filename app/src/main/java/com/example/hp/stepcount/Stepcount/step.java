package com.example.hp.stepcount.Stepcount;
...
public class step {
    ...
    static final int valueNum = 4;
    //用于存放计算阈值的波峰波谷差值
    static float[] tempValue = new float[valueNum];
    static int tempCount = 0;
    //是否上升的标志位
    static boolean isDirectionUp = false;
    //持续上升次数
    static int continueUpCount = 0;
    ...
    //当前的时间
    static long timeOfNow = 0;
    //当前传感器的值
    float gravityNew = 0;
    //上次传感器的值
    static float gravityOld = 0;
    //动态阈值需要动态的数据，这个值用于这些动态数据的阈值
    static final float initialValue = (float) 1.3;
    //初始阈值
    static float ThreadValue = (float) 2.0;
     ...
     * */
    public static int DetectorNewStep(float values) {
        int steppp = 0;
        if (gravityOld == 0) {
            gravityOld = values;
        } else {
